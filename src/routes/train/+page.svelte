<script>
  ...
  const nextYear = (val) => {
    if(soluce[val].some(x=>x===curYear)) {
...
<Page> 
  <Block class="flex flex-col space-x-4 h-[calc(100vh-74px)]">
    <Block class="flex flex-col w-full h-1/5 justify-center space-y-4">
      <p class="text-4xl self-center text-center w-full">
        {curYear}
      </p>
      <p class="text-2xl self-center text-center w-full">
        {yearList.length - 1} left
      </p>
    </Block>
    <Block class="flex flex-col justify-around p-0 m-0 h-4/5 w-4/5 self-center">
      {#each [...Array(7).keys()] as val}
        <Button large align-middle class="w-full text-white font-bold py-2 px-4 rounded h-12" 
          onClick={()=>nextYear(val)}>{val}
        </Button>
      {/each}
    </Block>
  </Block>
